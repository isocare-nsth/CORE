...
namespace Saving.Applications.shrlon.ws_sl_contract_adjust_ctrl
{
    public partial class ws_sl_contract_adjust : PageWebSheet, WebSheet
    {
        ...
        public decimal GetIntrate(string coopid, string loancontractno)
        {
            ...
            string sql = @"select lncfloanintratedet.interest_rate as intrate  from lncontmaster 
left join lnloantype on lncontmaster.loantype_code = lnloantype.loantype_code 
left join lncfloanintrate on lnloantype.inttabrate_code = lncfloanintrate.loanintrate_code and lnloantype.coop_id = lncfloanintrate.coop_id
                            left join lncfloanintratedet on lncfloanintrate.loanintrate_code = lncfloanintratedet.loanintrate_code and lncfloanintrate.coop_id = lncfloanintratedet.coop_id and getdate() between effective_date and expire_date
                            left join lnloantypeperiod on lncontmaster.loantype_code = lnloantypeperiod.loantype_code and lncontmaster.principal_balance between money_from and money_to 
