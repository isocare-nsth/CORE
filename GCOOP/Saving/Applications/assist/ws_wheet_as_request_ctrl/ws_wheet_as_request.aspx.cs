    ...
    public partial class ws_wheet_as_request : PageWebSheet, WebSheet
    {
        ...
        public void CheckJsPostBack(string eventArg)
        {
            ...
            else if (eventArg == PostCardPerson)
            {
                ...
                sqlStr = WebUtil.SQLFormat(sqlStr, state.SsCoopId, dsFamilydecease.DATA[0].card_person);
                Sdt dt1 = WebUtil.QuerySdt(sqlStr);
                if (dt1.Next())
                {
                    dsFamilydecease.DATA[0].card_person = "";
                    LtServerMessage.Text = WebUtil.ErrorMessage("เลขบัตรบัตรประชาชน " + dsFamilydecease.DATA[0].card_person + " เป็นของเลขสมาชิก " + dt1.GetString("member_no") + " ซึ่งสมาชิกจะไม่สามารถขอสวัสดิการประเภทที่ได้");
                    return;
                }
                //////////////////////////////////////////////////
                ...
                sqlStr = WebUtil.SQLFormat(sqlStr, state.SsCoopControl, li_year, dsFamilydecease.DATA[0].card_person, dsAssisttype.DATA[0].assisttype_code);
        ...
        private void GetTap()
        {
            ...
            sqlStr = @"select
	                    ...
	                    req.assist_year,
	                    req.assisttype_code,
	                    case when assucfassisttype.process_flag = 0 then 2 else assucfassisttype.process_flag end process_flag,
	                    typedet.num_pay
        ...
        private void RetriveData()
        {
            ...
            string sql = @"select  * from
                (
	                select 
		                count(*) chk_mem, resign_status, nvl(dead_status, 0) dead_status
	                from mbmembmaster where coop_id = {0} and member_no = {1} group by resign_status, dead_status
	                union
	                select 
		                0, 0, 0 
            ...
            if(dt.GetInt32("resign_status") == 1){
                dsMain.DATA[0].member_no = "";
        ...
        private void chkFamiyReq()//kkkk
        {
            ...
            string sql = @"
                ...
                a.coop_control = {0} and a.assisttype_code = {1} and b.assisttype_pay = {2}";
            sql = WebUtil.SQLFormat(sql, state.SsCoopControl, dsAssisttype.DATA[0].assisttype_code, dsFamilydecease.DATA[0].ASSISTPAY_CODE);
            Sdt dt = WebUtil.QuerySdt(sql);
        ...
        public void SaveWebSheet()
        {
            ...
            try
            {
                ...
                else if (ls_typegroup == "03")//ครอบครัวเสียชีวิต
                {
                    ...
                    try
                    {
                        if (dsFamilydecease.DATA[0].card_person == "")
                        {
                            LtServerMessage.Text = WebUtil.ErrorMessage("กรุณากรอก เลขบัตรประชาชน");
                            return;
                        }
                        ls_family_card = dsFamilydecease.DATA[0].card_person;
                    }
                ...
                if (ls_assdoc == "")
                {
                    ...
                    sqlStr = WebUtil.SQLFormat(sqlStr
                        ...
                        , ls_treatstart_date, ls_treatend_date, ls_family_card, dec_damages, ls_mem_card);
        ...
        public void CelTreatDate()
        {
            ...
            if (dt.Next())// เช็ดวันที่รักษาอยู่ในปีสวัสดิการนี้หรือไม่
            {
                ...
                {
                    decimal use_allday = dt2.GetInt32("use_day") / 5;
                ...
                dsPatronize.DATA[0].cal_date = li_caltreat;
