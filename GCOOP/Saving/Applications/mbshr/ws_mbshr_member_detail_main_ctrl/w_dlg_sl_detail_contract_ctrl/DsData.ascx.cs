...
lnucfloanobjective.loanobjective_code+' '+lnucfloanobjective.loanobjective_desc as loanobjective_desc  
                        from lncontmaster 
                            left join lnucfcontlaw on lncontmaster.contlaw_status = lnucfcontlaw.contlaw_status
                            left join lnucfloanobjective on lncontmaster.loanobjective_code = lnucfloanobjective.loanobjective_code and lncontmaster.loantype_code = lnucfloanobjective.loantype_code
        ...
        public void DdLoancontlaw()
        {
            string sql = @"select contlaw_status,   CONVERT(varchar(2),contlaw_status) +' - '+contlaw_desc as display, 1 as sorter
                from lnucfcontlaw
                union
                select 0 as contlaw_status,'',0 from dual order by sorter, contlaw_status
                ";
            sql = WebUtil.SQLFormat(sql);
            DataTable dt = WebUtil.Query(sql);
            this.DropDownDataBind(dt, "contlaw_status", "display", "contlaw_status");
        }
    }
}