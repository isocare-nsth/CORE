    ...
    public partial class w_sheet_paychq_fromslip : PageWebSheet, WebSheet
    {
        ...
        public void SaveWebSheet()
        {
            ...
            try
            {
                ...
                string deletetemp = @"DELETE FROM fntemp 
                                                WHERE 
                                                entry_id ={1} and
                                                date_onchq = {2}and 
                                                coop_id = {0}";
                deletetemp = WebUtil.SQLFormat(deletetemp, state.SsCoopControl, as_entry, as_adtmtdate);
                //WebUtil.ExeSQL(update);
                exe.SQL.Add(deletetemp);
                ...
                for (int i = 0; i < row; i++)
                {
                    ...
                    if (choose_flag == 1)
                    {
                        ...
                        string insertemp = @"INSERT INTO fntemp 
		                                    (coop_id, chequebook_no, cheque_no, date_onchq, to_whom, money_amt, entry_id)
                                            VALUES ({0}, {1}, {2}, {3}, {4},{5},{6} )";
                        insertemp = WebUtil.SQLFormat(insertemp, state.SsCoopControl, as_chqbookno, as_chqstart_no, adtm_wdate, ls_paywhom, ldc_itemamt,as_entry);
                        //WebUtil.ExeSQL(update);
                        exe.SQL.Add(insertemp);
                    }
                }
                exe.Execute();
                exe.SQL.Clear();
