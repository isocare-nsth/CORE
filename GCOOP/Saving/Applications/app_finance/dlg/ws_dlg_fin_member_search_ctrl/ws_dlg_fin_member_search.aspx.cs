...
namespace Saving.Applications.app_finance.dlg.ws_dlg_fin_member_search_ctrl
{
    public partial class ws_dlg_fin_member_search : PageWebDialog, WebDialog
    {
        ...
        protected void BtSearch_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //ls_memno = WebUtil.MemberNoFormat(dsCriteria.DATA[0].member_no.Trim());
                //dsCriteria.DATA[0].member_no = ls_memno;
            ...
            {
                if (dsList.DATA[i].RESIGN_STATUS == 1)
                {
                    dsList.FindTextBox(i, "MEMBER_NO").BackColor = Color.Red;
                    dsList.FindTextBox(i, "FULLNAME").BackColor = Color.Red;
                    dsList.FindTextBox(i, "FULLMEMBGROUP").BackColor = Color.Red;
                }
                else
                {
                    dsList.FindTextBox(i, "MEMBER_NO").BackColor = Color.White;
                    dsList.FindTextBox(i, "FULLNAME").BackColor = Color.White;
                    dsList.FindTextBox(i, "FULLMEMBGROUP").BackColor = Color.White;
                }
            }
        }
    }
